...
public class TellstickDeviceListener extends Thread implements Runnable {
    ...
    public void run() {
        ...
        JNA.CLibrary.TDDeviceEvent fn = new JNA.CLibrary.TDDeviceEvent() {
            ...
            public void invoke(int deviceId, int method, Pointer data, int callbackId, Pointer context)
                ...
                try {
                    ...
                    TellstickDeviceEvent event = new TellstickDeviceEvent(ts, Method.getMethodById(method),
                            ChangeType.UPDATED, dataStr, System.currentTimeMillis());
        ...
        JNA.CLibrary.TDDeviceChangeEvent fn2 = new JNA.CLibrary.TDDeviceChangeEvent() {
            ...
            public void invoke(int deviceId, int method, int changeType, int callbackId, Pointer context)
                ...
                try {
                    ...
                    TellstickDeviceEvent event = new TellstickDeviceEvent(ts, null,
                            ChangeType.getChangeTypeById(method), null, System.currentTimeMillis());
                    changeListener.onRequest(event);
                    // The device is not supported.
                } catch (DeviceNotSupportedException e) {
                    ...
                    if (method == JNA.CLibrary.TELLSTICK_DEVICE_REMOVED) {
                        for (int i = 0; i < list.size(); i++) {
                            if (list.get(i).getId() == deviceId) {
                                list.remove(i);
                                TellstickDeviceEvent event = new TellstickDeviceEvent(null, null, ChangeType.REMOVED,
        ...
        int handle2 = JNA.CLibrary.INSTANCE.tdRegisterDeviceChangeEvent(fn2, null);
